...
export const signIn = async function (req, res, next) {
    ...
    if (!email || !password) return next(new HttpError('Missing email or password', 400));
    const user = await User.findOne({ email });      // check if the user already exists

    if (!user) return next(new HttpError('Invalid email address', 404));
...
export const signUp = async function (req, res, next) {
    ...
    if (existingUser) return next(new HttpError('The email address already exists', 409));
...
export const deleteAccount = async function (req, res, next) {
    if (!req.user || !req.user.id) return next(new HttpError('Unauthorized', 401));

    const { id, role } = req.user;
    if (!isValidObjectId(id)) return next(new HttpError('Invalid ID', 400));

    if (role === 'admin') return next(new HttpError('Admin account cannot be deleted', 403));
